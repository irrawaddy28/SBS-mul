...
if [ $# != 2 ]; then
	echo "usage: $0 <train lang> <test lang>"
	echo "example: $0 \"AR CA HG MD UR\" \"SW\" "
fi
...
gmmdir=exp/tri3b/${TEST_LANG}
alidir=exp/tri3b_ali/${TEST_LANG}
data_fmllr=data-fmllr-tri3b/${TEST_LANG}
...
if [ $stage -le 1 ]; then
  ...
  steps/nnet/make_fmllr_feats.sh --nj $feats_nj --cmd "$train_cmd" \
     ...
     $dir data/${UNILANG_CODE}/train $gmmdir $dir/log $dir/data || exit 1
  steps/compute_cmvn_stats.sh $dir $dir/log $dir/data || exit 1;
  utils/validate_data_dir.sh --no-text $dir   
...
if [[ ! -z ${precomp_dbn} ]]; then
	initdir=exp/dnn4_pretrain-dbn/${TEST_LANG}/outdbn  #out-of-domain dbn (dbn learned from a corpus different than target corpus)
elif [[ ! -z ${precomp_dnn} ]]; then
	initdir=exp/dnn4_pretrain-dbn/${TEST_LANG}/outdnn  #out-of-domain dnn (dnn learned from a corpus different than  target corpus)
else
	initdir=exp/dnn4_pretrain-dbn/${TEST_LANG}/indbn #in-domain dbn (dbn learned from this target corpus)
fi
if [ $stage -le 2 ]; then
# First check for pre-computed DBN dir. Then try pre-computed DNN dir. If both fail, generate DBN now.
  ...
  if [[ ! -z ${precomp_dbn} ]]; then
	...
	$cuda_cmd $initdir/log/pretrain_dbn.log \
		...
		--rbm-iter 20 $data_fmllr/${UNILANG_CODE}/train $initdir || exit 1;  
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  for L in ${TRAIN_LANG} ${TEST_LANG}; do
    ...
    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.2 \
	  $graph_dir $data_fmllr/${TEST_LANG}/$L/dev $dir/decode_dev_text_G_$L &	
    steps/nnet/decode.sh --nj 4 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.2 \
      $graph_dir $data_fmllr/${TEST_LANG}/$L/eval $dir/decode_eval_text_G_$L &
